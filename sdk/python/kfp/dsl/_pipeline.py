...
def get_pipeline_conf():
    """
    Configure the pipeline level setting to the current pipeline将管道级别设置配置为当前管道
...
class Pipeline():
    ...
    def get_pipeline_functions():
        """
        获取所有导入过pp（使用装饰器）的函数Get all imported pipeline functions (decorated with @pipeline)."""
        return Pipeline._pipeline_functions

    @staticmethod
    def add_pipeline(name, description, func):
        """Add a pipeline function (decorated with @pipeline).
        增加一个pp函数 到指定的字典内 _pipeline_functions 形如{func: (name, description)}
        func 是一个函数的引用
        """
