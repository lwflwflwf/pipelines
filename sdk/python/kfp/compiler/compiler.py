...
class Compiler(object):
    ...
    def _process_args(self, raw_args, argument_inputs):
        ...
        for i, _ in enumerate(processed_args):
            ...
            matches = []
            match = re.findall(r'{{pipelineparam:op=([\w\s\_-]*);name=([\w\s\_-]+);value=(.*?)}}',
                               str(processed_args[i]))
            matches += match
            unsanitized_argument_inputs = {}
            for x in list(set(matches)):
                sanitized_str = str(
                    dsl.PipelineParam(K8sHelper.sanitize_k8s_name(x[1]), K8sHelper.sanitize_k8s_name(x[0]), x[2]))
            ...
            if argument_inputs:
                for param in argument_inputs:
                    if str(param) in unsanitized_argument_inputs:
                        full_name = self._pipelineparam_full_name(param)
                        processed_args[i] = re.sub(unsanitized_argument_inputs[str(param)],
